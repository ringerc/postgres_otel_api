...
**In-tree** (preferred): with the extensions dropped into a postgres
source tree, run `make check-world` or `meson test` from the postgres
root. The postgres build infrastructure handles temp-install setup,
`shared_preload_libraries` configuration, etc.

**Out-of-tree** (this repo): PGXS's standard `make installcheck`
does not run these TAPs because `NO_INSTALLCHECK = 1` (the suites
require `shared_preload_libraries`, which a plain installcheck can't
configure). A standalone TAP runner that handles this case is a
**TODO** — for now, exercise the TAPs via the in-tree path.
